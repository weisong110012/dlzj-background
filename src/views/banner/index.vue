  ...
  <div class="wrap">
    <el-card class="box-card">
      <el-row style="text-align: right;padding-bottom: 18px;">
        <template v-if="device !== 'mobile'">
          <el-input v-model="in_search" placeholder="请输入轮播图名称" prefix-icon="el-icon-search" style="display: inline-block;width: 250px;margin-right: 12px;" @input="fetchData" />
        </template>
        <el-button type="primary" @click="handleAddClick">新增轮播图</el-button>
      </el-row>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType === 'edit' ? '编辑公告' : '新增公告'" :fullscreen="device == 'mobile'">
      <el-form :model="noticeModel" label-width="80px" label-position="right">
        ...
        <el-form-item label="名称"><el-input v-model="noticeModel.title" /></el-form-item>
      </el-form>
...
<script>
import { getList, add, getdetail, edit, remove } from '@/api/banner'
...
export default {
  ...
  methods: {
    ...
    handleDeleteClick(item) {
      remove({ id: item.id.toString() }).then(response => {
