    ...
    <el-card class="box-card">
      <el-row style="text-align: right;padding-bottom: 18px;">
        <template v-if="device !== 'mobile'">
          <el-input v-model="in_search" placeholder="请输入用户名称" prefix-icon="el-icon-search" style="display: inline-block;width: 250px;margin-right: 12px;" @input="fetchData" />
        </template>
      ...
      <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" prop="created_at" label="用户手机号">
          <template slot-scope="scope">
            <span>{{ scope.row.phone }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="created_at" label="用户类型">
          <template slot-scope="scope">
            <span v-if='scope.row.type==1'>施工方</span>
            <span v-else-if='scope.row.type==2'>租赁公司</span>
            <span v-else-if='scope.row.type==3'>安装队</span>
            <span v-else-if='scope.row.type==4'>师傅</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="created_at" label="实名认证信息">
          <template slot-scope="scope">
            <span>查看</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="created_at" label="安全认证信息">
          <template slot-scope="scope">
            <span>{{ scope.row.is_safety==1?'已认证':'未认证' }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="created_at" label="资质认证信息">
          <template slot-scope="scope">
            <span>{{ scope.row.is_safety==1?'已认证':'未认证' }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="created_at" label="官方认证信息">
          <template slot-scope="scope">
            <span>{{ scope.row.is_safety==1?'已认证':'未认证' }}</span>
          </template>
        </el-table-column>
